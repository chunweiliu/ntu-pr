...
for tt = 1:length(testsets)
    ...
    for m = 1:length(dims)
        ...
        for n = 1:length(sr)
            ...
            [FRModel, disthd] = FRTrain_pca( trainlist, gallerylist, r, c, d );

            accuracy(m, n) = pca_accur( FRModel, disthd, testlist );
    ...
    %legend('50', '100');
    title( 'The relationship between patch size, dimensionality and accuracy using PCA' );
    xlabel('Log patch size');
    ylabel('Accuracy');

    filename = sprintf('../public-html/show/pr/pca_patch_%d.png', tt);
    saveas(gcf, filename, 'png');
    hold off;

    matname = sprintf('pca/accuracy_patch_%d.mat', tt);
