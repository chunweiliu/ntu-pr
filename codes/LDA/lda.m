...
% Modifications:
% 22-may-2009, Chun-Wei, let y can be any numbers
% 05-oct-2006, VF
% 25-may-2004, VF
% 3-may-2004, VF
...
%echo(' scatter matrix constructed ...');
%tic;
for i = 1:nclass,
% for i = labels
  inx_i = find( data.y==labels(i));
  X_i = data.X(:,inx_i); 
  mean_Xi = mean(X_i,2);
  Sw = Sw + cov( X_i', 1);
...
%toc;
%echo(' scatter matrix done ! ' );

% Compute projection matrix
%[U,D,V2]=svd( inv( Sw )*Sb ); ERROR !
%model.W = V(:,1:new_dim);

%fprintf(' eigenvalue solving ...\n');
%tic;
[V,D]=eig( inv( Sw )*Sb );
[D,inx] = sort(diag(D),1,'descend');
%toc;
%fprintf(' eigenvalue done !\n');
